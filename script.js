...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function obtenerRespuestas() {
    ...
    inputs.forEach(function(input, index) {
      ...
      respuestas.push(`${numeroRespuesta}. ${input.previousElementSibling.textContent} ${input.value}`);
    });
  ...
  function enviarWhatsapp(mensaje) {
    const numeroComercio = '3413507933'; // Aquí colocas el número del comercio
    const url = `https://wa.me/${numeroComercio}?text=${encodeURIComponent(mensaje)}`;
